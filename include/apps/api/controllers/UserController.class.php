...
class UserController extends CommonController {
	...
	public function getorder(){
		...
		$order = array();
		$pay_num = $this->model->query("SELECT count(*) as num FROM " . $this->model->pre . "order_info WHERE user_id = '$user_id' " . 'and pay_status = ' . PS_UNPAYED . ' and order_status not in(' . OS_CANCELED . ','. OS_INVALID .','. OS_RETURNED .')');
		$order['pay_num'] = intval($pay_num[0]['num']);
		$rec_num = $this->model->query("SELECT count(*) as num FROM " . $this->model->pre . "order_info WHERE user_id = '$user_id' and shipping_status = " . SS_SHIPPED );
		$order['rec_num'] = intval($rec_num[0]['num']);
		$finish_num = $this->model->query("SELECT count(*) as num FROM " . $this->model->pre . "order_info WHERE user_id = '$user_id' and shipping_status = " . SS_RECEIVED);
		$order['finish_num'] = intval($finish_num[0]['num']);
		$refund_num = $this->model->query("SELECT count(*) as num FROM " . $this->model->pre . "order_info WHERE user_id = '$user_id' and order_status = " . OS_RETURNED );
		$order['refund_num'] = intval($refund_num[0]['num']);
