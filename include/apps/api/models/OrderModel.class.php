...
class OrderModel extends BaseModel {
     ...
     * @return  boolean
     */
    function addto_cart($user_id,$goods_id, $num = 1, $spec = array(), $parent = 0) {
        ECTouch::err()->clean();
        $_parent_id = $parent;
		/* 取得商品信息 */
        ...
        if (empty($goods)) {
            $res['content'] = L('goods_not_exists');
			$res['suc'] = 0;
            return $res;
        }
        ...
        if ($num > 0) {
            ...
            if ($row) { //如果购物车已经有此物品，则更新
                ...
                if (C('use_storage') == 0 || $num <= $goods_storage) {
                    ...
                    $res['content'] = sprintf(L('shortage'), $num);
					$res['suc'] = 0;
					return $res;
                }
            } else { //购物车没有此物品，则插入
        ...
        /* 把赠品删除 */
        $sql = "DELETE FROM " . $this->pre . "cart WHERE user_id = '" . $user_id . "' AND is_gift <> 0";
        $this->query($sql);
		$res['content'] = '加入购物车成功';
		$res['suc'] = 1;
		return $res;
     ...
     * @return  array
     */
    function get_cart_goods($user_id) {
        /* 初始化 */
        $goods_list = array();
        $total = array(
