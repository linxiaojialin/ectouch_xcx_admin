			...
			<div class="swiper-slide">
				...
				<div class="select-two">
					<a class="select-title padding-all j-menu-select">
						...
						<?php $_from = $this->_var['brands']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'brand_0_02759700_1514339631');$this->_foreach['brand'] = array('total' => count($_from), 'iteration' => 0);
if ($this->_foreach['brand']['total'] > 0):
    foreach ($_from AS $this->_var['brand_0_02759700_1514339631']):
        $this->_foreach['brand']['iteration']++;
?> 
						<?php if ($this->_var['brand_0_02759700_1514339631']['brand_id'] == $this->_var['brand_id']): ?> 
						<?php echo $this->_var['brand_0_02759700_1514339631']['brand_name']; ?>
						<?php endif; ?> 
						<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
						</em><i class="iconfont icon-jiantou tf-180 ts-2"></i></span>
					</a>
					<ul class="j-sub-menu padding-all j-get-one" data-istrue="true">
						<?php $_from = $this->_var['brands']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'brand_0_02768800_1514339631');$this->_foreach['brands'] = array('total' => count($_from), 'iteration' => 0);
if ($this->_foreach['brands']['total'] > 0):
    foreach ($_from AS $this->_var['brand_0_02768800_1514339631']):
        $this->_foreach['brands']['iteration']++;
?>
						<li class="ect-select" id="brand-select">
							<label class="ts-1"><?php echo htmlspecialchars($this->_var['brand_0_02768800_1514339631']['brand_name']); ?>
								<i class="fr iconfont icon-gou ts-1"></i>
								<input type="hidden" value="<?php echo $this->_var['brand_0_02768800_1514339631']['brand_id']; ?>" />
							</label>
						</li>
						<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
					</ul>
